...
const AppScreen = ({
  ...
  const [videoPlaying,   setVideoPlaying]   = useState(false);
  const [controlEnabled, setControlEnabled] = useState(false);
  const [muted,          setMuted]          = useState(true);   // viewer mic — starts muted
  const [showToolbar,    setShowToolbar]     = useState(true);
  const [hostMinimized,  setHostMinimized]   = useState(false);
  ...
  // ── Attach remote stream ───────────────────────────────────────────────────
  // <video muted> shows the screen (no audio from it).
  // <audio> plays host audio (screen audio + host mic).
  //
  // FIX vs old code: We now set el.srcObject = stream directly instead of
  // creating new MediaStream(audioTracks). Reasons:
  //
  //   1. Audio elements natively ignore video tracks in a MediaStream — setting
  //      the full stream is safe and simpler.
  //
  //   2. The stream reference is a live object. If audio tracks arrive AFTER
  //      the stream event fires (addtrack), the element picks them up
  //      automatically because srcObject points to the live stream.
  //
  //   3. Eliminates the old bug where new MediaStream(audioTracks) captured a
  //      snapshot: if tracks.length===0, playAudio was never called; if more
  //      tracks arrived later, the audioOnly snapshot didn't include them.
  //
  //   4. Fixes the unlockAudio race: srcObject is set to the real `stream`
  //      object (not the unlockStream), so unlockAudio's .then() sees
  //      srcObject !== unlockStream and skips the null → audio survives.
  // ─────────────────────────────────────────────────────────────────────────
  const attachStream = useCallback((stream) => {
    if (!stream) return;

    // Video — muted so we control audio via the separate <audio> element
    if (videoRef.current) {
      videoRef.current.srcObject = stream;
      videoRef.current.muted     = true;
      videoRef.current.volume    = 0;
    ...
    // Audio — set the full stream directly on the element.
    // The audio element plays all audio tracks in the stream and ignores video.
    // This also works correctly when tracks arrive after the stream event.
    if (_audioRef.current) {
      const audioEl = _audioRef.current;
      console.log("🔊 Viewer: attaching audio stream, tracks:", stream.getTracks().map(t =>
        `${t.kind} label="${t.label}" en=${t.enabled} state=${t.readyState}`
      ));

      audioEl.srcObject = stream;   // ← live stream reference, not a snapshot
      audioEl.volume    = 1.0;
      audioEl.muted     = false;
      audioEl.play()
        .then(() => console.log("🔊 Viewer audio playing ✅"))
        .catch(e  => {
          console.warn("🔊 viewer audio.play() failed:", e.message);
          // Retry once after short delay — may have been blocked by browser policy
          setTimeout(() => {
            if (_audioRef.current?.srcObject) {
              _audioRef.current.play().catch(e2 => console.warn("🔊 retry failed:", e2.message));
            }
          }, 500);
        });
    }
  ...
  // Sync muted UI with track state.
  // Viewer mic is muted after 200ms in App.js — check at 300ms.
  useEffect(() => {
    const t = setTimeout(() => {
      const track = localMicTrackRef?.current;
      if (track) setMuted(!track.enabled);
  ...
  // ── Mute / unmute YOUR mic (viewer → host direction) ──────────────────────
  // track.enabled gates the RTCRtpSender. true = host can hear viewer.
  const toggleMute = () => {
    const track = localMicTrackRef?.current;
    if (!track) { console.warn("🎤 No mic track"); return; }
    track.enabled = !track.enabled;
  ...
  return (
    <div style={{ width:"100vw", height:"100vh", background:"#0a0a0a", display:"flex", flexDirection:"column", overflow:"hidden" }}>
      ...
      {/* Audio provided by App.js via audioRef — permanently mounted there */}
      ...
      <div style={{ flexShrink:0, overflow:"hidden", height:showToolbar?54:0, transition:"height 0.2s ease", display:"flex", alignItems:"center", justifyContent:"space-between", padding:showToolbar?"0 14px":0, background:"rgba(13,13,18,0.98)", borderBottom:showToolbar?"1px solid rgba(255,255,255,0.07)":"none" }}>
        ...
        <div style={{ display:"flex", gap:6, alignItems:"center" }}>
          ...
          {/* ── VIEWER MIC BUTTON — always visible in toolbar ── */}
          <button onClick={toggleMute} title={muted?"Unmute mic — let host hear you":"Mute your mic"} style={tbtn(muted?"#374151":"#059669")}>
            {muted
              ? <svg style={{width:13,height:13}} fill="none" viewBox="0 0 24 24" stroke="currentColor">
