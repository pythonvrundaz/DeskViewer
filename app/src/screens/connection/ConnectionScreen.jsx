...
// Renders a contextual error with icon and dismiss button.
// type: "unavailable" | "rejected" | "disconnected" | "error"
const ErrorBanner = ({ error, onDismiss }) => {
  if (!error) return null;
  const configs = {
    unavailable:  { bg:"#fef2f2", border:"#fca5a5", icon:"📵", title:"Host Not Online",       color:"#991b1b" },
    rejected:     { bg:"#fef2f2", border:"#fca5a5", icon:"❌", title:"Connection Rejected",    color:"#991b1b" },
    disconnected: { bg:"#fffbeb", border:"#fcd34d", icon:"🔌", title:"Session Ended",          color:"#92400e" },
    error:        { bg:"#fef2f2", border:"#fca5a5", icon:"⚠️", title:"Connection Error",       color:"#991b1b" },
    timeout:      { bg:"#fef2f2", border:"#fca5a5", icon:"⏱️", title:"Connection Timed Out",   color:"#991b1b" },
  };
  const cfg = configs[error.type] || configs.error;
  return (
    <div style={{position:"fixed",top:20,left:"50%",transform:"translateX(-50%)",zIndex:200,
      background:cfg.bg,border:`1.5px solid ${cfg.border}`,borderRadius:12,padding:"14px 20px",
      display:"flex",alignItems:"flex-start",gap:12,boxShadow:"0 4px 24px rgba(0,0,0,0.15)",
      maxWidth:420,width:"calc(100% - 40px)"}}>
      <span style={{fontSize:22,lineHeight:1,flexShrink:0}}>{cfg.icon}</span>
      <div style={{flex:1}}>
        <div style={{fontWeight:700,color:cfg.color,fontSize:14,marginBottom:2}}>{cfg.title}</div>
        <div style={{color:cfg.color,fontSize:12,opacity:0.85}}>{error.message}</div>
      </div>
      <button onClick={onDismiss} style={{background:"none",border:"none",cursor:"pointer",
        color:cfg.color,fontSize:18,lineHeight:1,padding:"0 0 0 8px",opacity:0.6,flexShrink:0}}
...
const ConnectionScreen = ({
  ...
  hostMicGainRef, localStreamRef, callRef,
  incomingCall, incomingCallerId, acceptCall, rejectCall,
  startCall, cancelConnecting, onEndSession,
  callRejected, sessionReset,
  ...
  const scheduleHide = useCallback(() => {
    ...
    if (!annoMode) toolbarTimerRef.current = setTimeout(() => setShowToolbar(false), 3000);
  }, [annoMode]);
  ...
  // ── Session reset — triggered by App.js when any session ends ─────────────
  useEffect(() => {
  ...
  // CORNER CASE: callRejected received — reset connecting state
  // Previously this effect only reset connecting locally, but the mic stream
  // and call object were still alive. Now App.js handles cleanup there;
  // here we just clear the UI state.
  useEffect(() => {
    if (callRejected) { setConnecting(false); setRemoteId(""); }
  }, [callRejected]);
  ...
  useEffect(() => {
    ...
    const onClip = ({ text }) => {
      if (!text) return;
      ipcRenderer.invoke("WRITE_CLIPBOARD", text).catch(() => navigator.clipboard.writeText(text).catch(()=>{}));
  ...
  useEffect(() => {
    ...
    const onCopy = async () => {
      try {
        ...
        if (s?.connected&&rid) { s.emit("clipboard-sync",{remoteId:rid,text}); showClipToast("📋 Clipboard sent to viewer"); }
      } catch {}
    };
  ...
  const toggleMute = () => {
    ...
    const nowMuted = gain.gain.value===0; gain.gain.value = nowMuted?1:0; setMuted(!nowMuted);
  };
  ...
  const handleConnect = (id = remoteId) => {
    ...
    // Input validation — these all show inline errors, not alerts
    if (!rid) return;
    if (!rid.match(/^\d+$/)) { return; }           // non-numeric — input type=tel prevents most
    if (rid.length !== 10)   { return; }           // enforced by maxLength
    if (rid === String(myId)) { return; }          // can't connect to self — button disabled
    setConnecting(true);
    // Save to recent AFTER validation passes
    setRecentList(saveRecent(rid));
    startCall(rid);
  };
  ...
  const startRecording = useCallback(() => {
    ...
    try {
      ...
      rec.onstop=()=>{
        const blob=new Blob(recChunksRef.current,{type:mimeType}); const url=URL.createObjectURL(blob);
        const a=document.createElement("a"); a.href=url;
        a.download=`DeskViewer-Host-${new Date().toISOString().replace(/[:.]/g,"-").slice(0,-5)}.webm`;
        document.body.appendChild(a); a.click(); document.body.removeChild(a);
        setTimeout(()=>URL.revokeObjectURL(url),5000);
      };
      rec.start(1000); recorderRef.current=rec; setRecording(true); setRecDuration(0);
  ...
  useEffect(() => {
      ...
      try {
        ...
        stats.forEach(r=>{
          ...
          if(r.type==="outbound-rtp"&&r.kind==="video"){
            fps=r.framesPerSecond??null; const now=Date.now();
    ...
    const iv=setInterval(poll,2000); poll(); return ()=>clearInterval(iv);
  ...
  const syncCanvasSize = useCallback(() => {
    ...
    if (c.width!==w||c.height!==h) {
      ...
      tmp.getContext("2d").drawImage(c,0,0); c.width=w; c.height=h; c.getContext("2d").drawImage(tmp,0,0);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    socket.on("annotation-frame",onFrame); return ()=>socket.off("annotation-frame",onFrame);
  }, [socketRef]);
  ...
  const annoPointerDown = useCallback((e) => {
    if (!annoMode) return; e.preventDefault(); e.stopPropagation();
    const {x,y}=getCanvasXY(e); const c=canvasRef.current; if (!c) return;
  ...
  const annoPointerMove = useCallback((e) => {
    if (!annoDrawingRef.current||!annoMode) return; e.preventDefault();
    const {x,y}=getCanvasXY(e); const c=canvasRef.current; if (!c) return;
    const ctx=c.getContext("2d"); const {x:sx,y:sy}=annoStartRef.current;
    if (annoTool==="pen") { ctx.lineTo(x,y); ctx.stroke(); }
      ...
      if (annoTool==="rect") { ctx.beginPath(); ctx.strokeRect(sx,sy,x-sx,y-sy); }
        ...
        const dx=x-sx,dy=y-sy,angle=Math.atan2(dy,dx),len=Math.sqrt(dx*dx+dy*dy),hw=Math.min(len*0.35,18),ha=0.45;
  ...
  const annoCommitText = () => {
    ...
    const {x,y}=annoTextPosRef.current; const c=canvasRef.current; if (!c) return;
    const ctx=c.getContext("2d"); ctx.font=`${Math.max(annoSize*4,16)}px sans-serif`; ctx.fillStyle=HOST_COLOR;
    ctx.fillText(text,x,y); setAnnoTextVal(""); annoTextPosRef.current=null;
    const s=socketRef?.current, rid=String(remoteIdRef?.current||""), uid=String(userIdRef?.current||"");
    if (s?.connected&&rid) s.emit("annotation-frame",{remoteId:rid,userId:uid,role:"host",dataUrl:c.toDataURL("image/png")});
  };
  ...
  if (sessionActive) {
    return (
      <div style={{width:"100vw",height:"100vh",background:"#0a0a0a",display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        {clipToast&&<div style={{position:"fixed",top:16,left:"50%",transform:"translateX(-50%)",zIndex:400,background:"#1e3a5f",color:"#fff",fontSize:12,fontWeight:600,borderRadius:8,padding:"8px 16px",boxShadow:"0 4px 16px rgba(0,0,0,0.4)",pointerEvents:"none",whiteSpace:"nowrap"}}>{clipToast}</div>}
        ...
        <div style={{flexShrink:0,overflow:"hidden",height:showToolbar?54:0,transition:"height 0.2s ease",display:"flex",alignItems:"center",justifyContent:"space-between",padding:showToolbar?"0 14px":0,background:"rgba(10,20,40,0.98)",borderBottom:showToolbar?"1px solid rgba(59,130,246,0.2)":"none"}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            {recording&&<span style={{display:"flex",alignItems:"center",gap:5,background:"rgba(220,38,38,0.2)",border:"1px solid #ef4444",borderRadius:6,padding:"2px 8px",fontSize:11,fontWeight:700,color:"#fca5a5"}}><span style={{width:7,height:7,borderRadius:"50%",background:"#ef4444",animation:"pulse 1s infinite",display:"inline-block"}}/>REC {fmtDuration(recDuration)}</span>}
            {quality&&<span title={`RTT:${quality.rtt??'—'}ms | FPS:${quality.fps??'—'} | ${quality.kbps??'—'}kbps`} style={{display:"flex",alignItems:"center",gap:4,background:"rgba(255,255,255,0.05)",borderRadius:6,padding:"2px 8px",fontSize:11,fontWeight:600,color:qualityColor(),cursor:"default",border:`1px solid ${qualityColor()}44`}}><span style={{width:6,height:6,borderRadius:"50%",background:qualityColor()}}/>{qualityLabel()}{quality.rtt!=null&&<span style={{opacity:0.75}}>{quality.rtt}ms</span>}{quality.fps!=null&&<span style={{opacity:0.6}}>{quality.fps}fps</span>}</span>}
            {annoMode&&<span style={{display:"flex",alignItems:"center",gap:4,background:"rgba(59,130,246,0.25)",border:"1px solid rgba(59,130,246,0.4)",borderRadius:5,padding:"2px 8px",fontSize:10,fontWeight:700,color:"#bfdbfe"}}>✏️ ANNOTATING (blue)</span>}
          </div>
          <div style={{display:"flex",gap:6,alignItems:"center"}}>
            ...
            <button onClick={toggleMute} style={tbtn(muted?"#374151":"#059669")}>{muted?<svg style={{width:13,height:13}} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z"/><line x1="3" y1="3" x2="21" y2="21" strokeWidth={2} strokeLinecap="round"/></svg>:<svg style={{width:13,height:13}} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z"/></svg>}{muted?"Unmute Mic":"Mute Mic"}</button>
            <button onClick={()=>setAnnoMode(v=>!v)} style={tbtn(annoMode?"#1d4ed8":"#374151",annoMode)}><svg style={{width:13,height:13}} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>Annotate</button>
            <button onClick={()=>recording?stopRecording():startRecording()} style={tbtn(recording?"#dc2626":"#374151",recording)}>{recording?<svg style={{width:12,height:12}} viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="6" width="12" height="12" rx="1"/></svg>:<svg style={{width:13,height:13}} fill="none" viewBox="0 0 24 24" stroke="currentColor"><circle cx="12" cy="12" r="8" strokeWidth={2}/><circle cx="12" cy="12" r="3" fill="currentColor"/></svg>}{recording?"Stop Rec":"Record"}</button>
            ...
            <button onClick={()=>dispatch(setShowSessionDialog(true))} style={tbtn("#0284c7")}><svg style={{width:13,height:13}} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>Info</button>
            <button onClick={handleDisconnect} style={tbtn("#dc2626")}><svg style={{width:13,height:13}} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/></svg>Stop Sharing</button>
          </div>
        </div>
        ...
        {annoMode&&(
          <div style={{flexShrink:0,display:"flex",alignItems:"center",gap:10,padding:"6px 14px",background:"rgba(10,20,50,0.97)",borderBottom:"1px solid rgba(59,130,246,0.15)",flexWrap:"wrap"}}>
            <div style={{display:"flex",gap:4}}>
              {ANNO_TOOLS.map(t=><button key={t} onClick={()=>setAnnoTool(t)} style={{padding:"4px 10px",borderRadius:6,fontSize:11,fontWeight:600,cursor:"pointer",border:`1.5px solid ${annoTool===t?HOST_COLOR:"transparent"}`,background:annoTool===t?"#1d4ed8":"rgba(255,255,255,0.07)",color:"#fff"}}>{t==="pen"?"✏️ Pen":t==="arrow"?"➡️ Arrow":t==="rect"?"▭ Rect":t==="text"?"T Text":"◻ Erase"}</button>)}
            </div>
            <div style={{width:1,height:20,background:"rgba(255,255,255,0.15)"}}/>
            <div style={{display:"flex",alignItems:"center",gap:8,fontSize:11,color:"#9ca3af"}}>
              <span style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:10,height:10,borderRadius:"50%",background:HOST_COLOR,display:"inline-block",border:"1.5px solid #fff"}}/><span style={{color:"#93c5fd",fontWeight:600}}>You (blue)</span></span>
              <span style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:10,height:10,borderRadius:"50%",background:VIEWER_COLOR,display:"inline-block",border:"1.5px solid rgba(255,255,255,0.4)"}}/><span style={{color:"#fca5a5"}}>Viewer (red)</span></span>
            </div>
            <div style={{width:1,height:20,background:"rgba(255,255,255,0.15)"}}/>
            <div style={{display:"flex",gap:4,alignItems:"center"}}>
              {ANNO_SIZES.map(sz=><button key={sz} onClick={()=>setAnnoSize(sz)} style={{width:sz*2.5+10,height:sz*2.5+10,borderRadius:"50%",background:annoSize===sz?HOST_COLOR:"rgba(255,255,255,0.15)",border:annoSize===sz?"2px solid #93c5fd":"2px solid transparent",cursor:"pointer",padding:0,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{width:sz,height:sz,borderRadius:"50%",background:"#fff",display:"block"}}/></button>)}
            </div>
            {annoTool==="text"&&<input ref={annoInputRef} value={annoTextVal} onChange={e=>setAnnoTextVal(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();annoCommitText();}if(e.key==="Escape")setAnnoTextVal("");}} placeholder="Type then click to place…" style={{background:"rgba(255,255,255,0.1)",border:`1px solid ${HOST_COLOR}`,borderRadius:6,padding:"4px 8px",color:"#fff",fontSize:12,outline:"none",width:200}}/>}
            <button onClick={clearCanvas} style={{marginLeft:"auto",padding:"4px 12px",borderRadius:6,fontSize:11,fontWeight:600,cursor:"pointer",background:"rgba(220,38,38,0.3)",border:"1px solid #ef4444",color:"#fca5a5"}}>🗑 Clear Both</button>
          </div>
        )}
        ...
        <div style={{flex:1,display:"flex",overflow:"hidden"}}>
          ...
          {showChat&&(
            <div style={{width:300,display:"flex",flexDirection:"column",background:"#111827",borderLeft:"1px solid rgba(59,130,246,0.15)",flexShrink:0,position:"relative"}}>
              ...
              {uploading&&<div style={{padding:"4px 14px",color:"#60a5fa",fontSize:11}}>Uploading...</div>}
              {showEmoji&&<div style={{borderTop:"1px solid rgba(255,255,255,0.08)",padding:"8px",display:"flex",flexWrap:"wrap",gap:2,maxHeight:150,overflowY:"auto",background:"#1f2937"}}>{EMOJIS.map(e=><button key={e} onClick={()=>insertEmoji(e)} style={{background:"none",border:"none",fontSize:18,cursor:"pointer",padding:"3px 4px",borderRadius:4,lineHeight:1}}>{e}</button>)}</div>}
  ...
  // Input validation state (inline, not alerts)
  const remoteIdClean = remoteId.replace(/\D/g,"");
  const isSelf        = remoteIdClean === String(myId) && remoteIdClean.length === 10;
  const isValidLength = remoteIdClean.length === 10;
  const canConnect    = isValidLength && !isSelf && !connecting;

  // What to show below the input
  const inputError = isSelf          ? "You cannot connect to your own ID."
                   : remoteIdClean.length > 0 && !isValidLength ? `${remoteIdClean.length}/10 digits entered`
                   : null;
  ...
  const hs = {
    ...
    btn:   (bg,dis) => ({ width:"100%", padding:13, borderRadius:8, border:"none", background:dis?"#9ca3af":bg, color:"#fff", fontSize:15, fontWeight:700, cursor:dis?"not-allowed":"pointer", transition:"background 0.15s" }),
  };

  return (
    <div style={hs.page}>

      {/* ERROR BANNER — contextual errors from App.js (offline peer, network, etc.) */}
      <ErrorBanner error={connectError} onDismiss={clearConnectError}/>

      {/* REJECTION BANNER — host explicitly rejected */}
      {callRejected&&(
      ...
      {/* CLIPBOARD TOAST */}
      {clipToast&&<div style={{position:"fixed",top:16,left:"50%",transform:"translateX(-50%)",zIndex:300,background:"#1f2937",color:"#fff",fontSize:12,fontWeight:600,borderRadius:8,padding:"8px 16px",boxShadow:"0 4px 16px rgba(0,0,0,0.3)",pointerEvents:"none",whiteSpace:"nowrap"}}>{clipToast}</div>}
      ...
      <div style={hs.right}>
        ...
        <div style={hs.field}>
          ...
          <input readOnly value={myId||"Connecting..."}
            ...
            style={{...hs.input,cursor:"pointer",background:"#f9fafb"}} title="Click to copy"/>
        </div>

        {/* Remote ID — only numeric, maxLength 10, inline validation */}
        <div style={hs.field}>
          ...
          <input
            ...
            style={{
              ...hs.input,
              borderColor: isSelf ? "#ef4444" : remoteIdClean.length===10 && !isSelf ? "#22c55e" : "#e5e7eb",
              opacity: connecting ? 0.6 : 1,
              cursor: connecting ? "not-allowed" : "text",
            }}
          />
          {/* Inline validation hint */}
          {inputError&&!connecting&&(
            <p style={{margin:"5px 0 0",fontSize:12,color: isSelf?"#dc2626":"#6b7280"}}>{inputError}</p>
          )}
        </div>
        ...
        <div style={hs.field}>
          {!connecting ? (
            <button
              ...
              disabled={!canConnect}
              style={hs.btn("#dc2626", !canConnect)}
            >
              Connect
            </button>
          ) : (
            <>
              <button disabled style={hs.btn("#dc2626", true)}>
                <span style={{display:"inline-flex",alignItems:"center",gap:8}}>
                  <svg style={{width:14,height:14,animation:"spin 1s linear infinite"}} viewBox="0 0 24 24" fill="none"><circle style={{opacity:0.25}} cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path style={{opacity:0.75}} fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg>
                  Waiting for host…
                </span>
              </button>
              {/* CORNER CASE: viewer can cancel while waiting */}
              <p style={{textAlign:"center",fontSize:12,color:"#6b7280",marginTop:8}}>
                Waiting for the host to accept…
                <button
                  onClick={() => { cancelConnecting(); }}
                  style={{marginLeft:8,background:"none",border:"none",color:"#dc2626",
                    cursor:"pointer",fontSize:12,fontWeight:600,padding:0,textDecoration:"underline"}}
                >
                  Cancel
                </button>
              </p>
