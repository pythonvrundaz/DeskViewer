...
const AppScreen = ({ remoteStream, remoteStreamRef, socketRef, callRef, remoteIdRef, onDisconnect, onEndSession }) => {
  ...
  const attachStream = useCallback((stream) => {
    ...
    console.log("Attaching stream, tracks:", stream.getTracks().map(t => t.kind));
    const video = videoRef.current;
    video.srcObject = stream;
    // FIX: Set muted via property, not attribute — React doesn't sync muted attr
    video.muted = muted;
    video.play()
      .then(() => { console.log("✅ Playing"); setVideoPlaying(true); })
  ...
  // FIX MUTE: Set directly on video element via ref — React's muted prop is broken
  const toggleMute = () => {
    const next = !muted;
    setMuted(next);
  ...
  useEffect(() => {
    ...
    const getCoords = (e) => {
      ...
      // Map viewer mouse position → host screen coordinates
    ...
    const onMouseMove  = (e) => { mousePosRef.current = getCoords(e); };
    const onMouseDown  = (e) => { e.preventDefault(); socket.emit("mousedown", { userId, remoteId, event: { button: e.button, ...getCoords(e) } }); };
    const onMouseUp    = (e) => { socket.emit("mouseup",   { userId, remoteId, event: { button: e.button, ...getCoords(e) } }); };
    const onDblClick   = (e) => { socket.emit("dblclick",  { userId, remoteId, event: getCoords(e) }); };
    const onWheel      = (e) => { e.preventDefault(); socket.emit("scroll", { userId, remoteId, event: { scroll: e.deltaY, ...getCoords(e) } }); };

    const onKeyDown    = (e) => {
      if (e.ctrlKey && e.shiftKey && e.key === "I") return; // keep DevTools
      if (e.key === "Escape") { setControlEnabled(false); return; }
      e.preventDefault();
      socket.emit("keydown", { userId, remoteId, event: { keyCode: e.key, ctrl: e.ctrlKey, shift: e.shiftKey, alt: e.altKey } });
    };
    const onKeyUp = (e) => { socket.emit("keyup", { userId, remoteId, event: { keyCode: e.key } }); };
  ...
  }, [controlEnabled, socketRef, userId, remoteId]);

  // FIX DISCONNECT: use remoteIdRef (always fresh) not redux state (may be stale)
  const handleDisconnect = () => {
    if (!window.confirm("End this session?")) return;
    const socket = socketRef.current;
    const rid = remoteIdRef?.current || remoteId;
    console.log("Disconnecting from:", rid);
  ...
  return (
    <div style={{ width: "100vw", height: "100vh", background: "#000", position: "relative", overflow: "hidden" }}>
      ...
      {/* Video element — no muted attribute, controlled via ref */}
      <video
        ref={videoRef}
        autoPlay
        playsInline
        style={{ width: "100%", height: "100%", objectFit: "contain", display: "block", cursor: controlEnabled ? "crosshair" : "default" }}
      />
      ...
      {videoPlaying && (
        <div style={{
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {/* Mute */}
            <button onClick={toggleMute} style={btn(muted ? "#6b7280" : "#0369a1")} title={muted ? "Unmute" : "Mute audio"}>
              {muted
                ...
                : <svg style={{width:15,height:15}} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.536 8.464a5 5 0 010 7.072M12 6v12m-3.536-9.536a5 5 0 000 7.072M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707A1 1 0 0112 5v14a1 1 0 01-1.707.707L5.586 15z"/>
                  </svg>
              }
              {muted ? "Unmute" : "Mute"}
            </button>
            ...
            {/* Remote Control */}
            <button onClick={() => setControlEnabled(c => !c)} style={btn(controlEnabled ? "#b91c1c" : "#7c3aed")}>
              <svg style={{width:15,height:15}} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {/* Session Info */}
            <button onClick={() => dispatch(setShowSessionDialog(true))} style={btn("#0284c7")}>
              <svg style={{width:15, height:15}} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {/* Disconnect */}
            <button onClick={handleDisconnect} style={btn("#dc2626")}>
              <svg style={{width:15,height:15}} fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {controlEnabled && videoPlaying && (
        <div style={{ position: "absolute", bottom: 20, left: "50%", transform: "translateX(-50%)", background: "rgba(185,28,28,0.9)", color: "#fff", padding: "6px 18px", borderRadius: 20, fontSize: 12, fontWeight: 600, zIndex: 20, display: "flex", alignItems: "center", gap: 6 }}>
